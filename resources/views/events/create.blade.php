                    ...
                    <form method="POST" action="{{ route('events.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="start" class="block mt-1 w-full" type="datetime-local" name="start" :value="old('start')" required />
                            ...
                            <div class="flex items-center">
                                <x-checkbox-input id="is_online" name="is_online" :checked="old('is_online', false)" />
                                <x-input-label for="is_online" :value="__('This is an online event')" class="ml-2" />
                            </div>

                            <!-- Meeting URL (shown only if online event is checked) -->
                            <div id="meeting_url_container" class="hidden">
                                <x-input-label for="meeting_url" :value="__('Meeting URL')" />
                                <x-text-input id="meeting_url" class="block mt-1 w-full" type="url" name="meeting_url" :value="old('meeting_url')" />
    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (checkbox.checked) {
                    document.getElementById('meeting_url_container').classList.remove('hidden');
                }
            });
        </script>
    @endpush
